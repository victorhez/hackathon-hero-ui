...
export function getCleanverseEnv(): CleanverseEnv {
  const baseUrl = process.env.CLEANVERSE_BASE_URL ?? "https://api.cleanverse.com";
...
function authHeaders(env: CleanverseEnv): HeadersInit {
  const token = Buffer.from(`${env.apiId}:${env.apiKey}`).toString("base64");
  return {
...
async function request<T>(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new CleanverseApiError(res.status, text);
    }
...
export function validateWallet(wallet: string): string {
  if (!wallet || typeof wallet !== "string") {
    throw new Error("Wallet address is required");
  }
  const trimmed = wallet.trim();
  if (!trimmed.startsWith("0x") || trimmed.length < 40) {
    throw new Error("Invalid wallet address: must start with 0x and be at least 40 characters");
  }
  return trimmed.toLowerCase();
}

export function deterministicWalletAgeDays(wallet: string): number {
  let hash = 0;
